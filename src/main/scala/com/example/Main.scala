...
/** HTTP Ranged Requests (Content-Range) endpoint and server logic with TAPIR
  */
def httpRangedVideoEndPoint: ServerEndpoint[Any, IO] = {
  val video2 = endpoint.get
    ...
    .out(
      header(
        sttp.model.Header
          .contentType(MediaType("video", "mp4", None, Map.empty))
      )
    )
  ...
  val video_play2 = video2.serverLogicSuccess((range: Option[String]) => {
    ...
    val rangeValue = for {
      ...
      Try(rangeMinMax(0).toLong).toOption,
      Try(rangeMinMax(1).toLong).toOption,
      file.length()
    ))
    ...
    rangeValue match {
      case Some(rv) =>
        IO(
          (
            "bytes",
            Some(
              s"bytes ${rv.start.getOrElse("")}-${rv.end.getOrElse("")}/${file.length()}"
            ),
      ...
      case None =>
        IO(
          ...
          sttp.tapir.InputStreamRange(
            () => fis,
            Some(RangeValue(Some(0L), Some(0L), file.length))
          )
        )
    }
  })
...
/** HTTP mp4 video file, endpoint and server logic with TAPIR, this won't work
  * on Safari
  */
def fileRetrievalAsMp4: ServerEndpoint[Any, IO] = {
  ...
  val video_play: ServerEndpoint[Fs2IOStreams, IO] =
    video.serverLogicSuccess[IO](Unit => {
...
/** Multipart GET with Tapir ( endpoint and server logic )
  */
def multiPart = {

  val bytesPart = Part("part1", "TEXT BODY YY-90".getBytes())
  val filePart = Part[java.io.File](
    "part2",
    java.io.File("web_root/quartz-h2.jpeg"),
  ...
  val mpart = endpoint.get
    .in("mpart")
    .out(
      multipartBody: EndpointIO.Body[Seq[RawPart], MultipartForm]
    ) // Seq[Part[Array[Byte]]]
...
/** Tapir endpoint and server logic for json GET
  */
def jsonGet = {
...
/** Tapir endpoint and server logic for json POST
  */
def jsonPost = {
...
def webSocket123 = {
  ...
  wse.serverLogicSuccess[IO](data => IO(wsPipe))
}

/** Simple Local Date Time with GET
  */
def ldt: ServerEndpoint[Any, IO] = endpoint.get
  .in("ldt")
  .out(stringBody)
  .serverLogic(Unit => IO(Right(new java.util.Date().toString())))

/** Simple Root endpoint for Tapir */
def top = endpoint.get
  .in("")
  .errorOut(stringBody)
  .out(stringBody)
...
object Main extends IOApp {
  ...
  def run(args: List[String]): IO[ExitCode] = {
    ...
    // http file server endpoint
    val docEndpoint = filesServerEndpoints[IO]("doc")("web_root/doc")

    // assemble our edpoints into list
    val serverEndpoints = List(
      top, // https://localhost:8443/
      ldt, // https://localhost:8443/ldt
      jsonGet, // https://localhost:8443/user
      jsonPost, // POST https://localhost:8443/user  - use JSON from GET, edit and test it with POST, server logic has println
      multiPart, // https://localhost:8443/mpart.html - how to test: insomnia HTTP client will interpret and show parts
      httpRangedVideoEndPoint, // https://localhost:8443/mp4safari
      fileRetrievalAsMp4, // https://localhost:8443/mp4
      webSocket123 //https://localhost:8443/doc/ws.html
    )

    val tapirInterpreterForQH2 =
      QuartzH2ServerInterpreter().toRoutes(serverEndpoints ++ docEndpoint)
    ...
    for {
      ...
      exitCode <- new QuartzH2Server(
        ...
        16000,
        Some(ctx)
      ) // use 0.0.0.0 for non-local exposure
        .start(tapirInterpreterForQH2, sync = false)
